...
type Transaction = { tx_hash?: string; from?: string; to?: string; amount?: number; asset?: string; timestamp?: string; source_chain?: string; destination_chain?: string; chain?: string; vasp?: string; entity_type?: string; explorer_url?: string }
...
const COLORS: Record<string, string> = { victim: '#f97316', intermediate: '#38bdf8', vasp: '#22c55e', mixer: '#f59e0b', cluster_member: '#8b5cf6', candidate: '#f43f5e' }
...
function fitViewport(layout: { width: number; height: number; positions: Map<string, { x: number; y: number }> }) {
  const points = [...layout.positions.values()]
  if (!points.length) return { zoom: 1, x: 0, y: 0 }
  const nodePadding = 56
  const minX = Math.min(...points.map((point) => point.x)) - nodePadding; const maxX = Math.max(...points.map((point) => point.x)) + nodePadding
  const minY = Math.min(...points.map((point) => point.y)) - nodePadding; const maxY = Math.max(...points.map((point) => point.y)) + nodePadding
  const width = Math.max(1, maxX - minX); const height = Math.max(1, maxY - minY)
  const zoom = Math.max(.65, Math.min(1.15, Math.min(layout.width / width, layout.height / height)))
  return { zoom, x: layout.width / 2 - ((minX + maxX) / 2) * zoom, y: layout.height / 2 - ((minY + maxY) / 2) * zoom }
}

export default function GraphView({ data, selectedTransactionHash, onTransactionSelect }: { data: any; selectedTransactionHash?: string | null; onTransactionSelect?: (transaction: Transaction) => void }) {
  ...
  const [selectedRelationship, setSelectedRelationshipState] = useState<string | null>(null)
  ...
  useEffect(() => {
    setSelectedNodeId(null); setSelectedRelationship(null); setShowAll(false); setActiveHop(null); setViewport({ zoom: 1, x: 0, y: 0 })
  }, [data])
  ...
  const model = useMemo(() => {
    ...
    const mixerIds = new Set(transactions.filter((transaction) => String(transaction.entity_type || '').toLowerCase() === 'mixer').flatMap((transaction) => [identity(transaction.from, transaction.source_chain || transaction.chain || chain), identity(transaction.to, transaction.destination_chain || transaction.chain || chain)]))
    ...
    const nodes: Node[] = [...ids].map((id) => {
      ...
      const type = id === source ? 'victim' : id === candidate ? 'candidate' : mixerIds.has(id) ? 'mixer' : vaspIds.has(id) ? 'vasp' : clusterById.has(id) ? 'cluster_member' : 'intermediate'
  ...
  const focusedRelationships = useMemo(() => activeHop === null ? visibleRelationships : visibleRelationships.filter((relationship) => relationship.hop === activeHop), [activeHop, visibleRelationships])
  const setSelectedRelationship = (relationshipId: string | null) => {
    setSelectedRelationshipState(relationshipId)
    const transaction = focusedRelationships.find((relationship) => relationship.id === relationshipId)?.transactions[0]
    if (transaction) onTransactionSelect?.(transaction)
  }

  const layout = useMemo(() => {
    const visibleIds = new Set(focusedRelationships.flatMap((relationship) => [relationship.source, relationship.target]))
    if (model.source) visibleIds.add(model.source)
    const nodes = model.nodes.filter((node) => visibleIds.has(node.id))
    const degree = new Map(nodes.map((node) => [node.id, focusedRelationships.filter((relationship) => relationship.source === node.id || relationship.target === node.id).length]))
      ...
      layerKeys.forEach((key) => {
        ...
        layer.sort((left, right) => {
          const center = (node: Node) => {
            const neighbours = focusedRelationships.filter((relationship) => relationship.target === node.id || relationship.source === node.id).map((relationship) => relationship.target === node.id ? relationship.source : relationship.target)
    ...
    bands.forEach((band, bandIndex) => {
      band.layer.forEach((node, index) => {
        if (compact) {
          positions.set(node.id, { x: bandIndex === 0 ? 100 : width - 100, y: height / 2 })
          return
        }
  ...
  const selectedEdge = focusedRelationships.find((relationship) => relationship.id === selectedRelationship) || null
  ...
  const selectedTransactionCount = focusedRelationships.reduce((sum, relationship) => sum + relationship.transactions.length, 0)
  const allVisible = visibleRelationships.length === model.relationships.length
  const visibleScope = `${focusedRelationships.length} relationship${focusedRelationships.length === 1 ? '' : 's'} · ${selectedTransactionCount} underlying transaction${selectedTransactionCount === 1 ? '' : 's'}`

  useEffect(() => {
    if (selectedNodeId && !layout.nodes.some((node) => node.id === selectedNodeId)) setSelectedNodeId(null)
    if (selectedRelationship && !focusedRelationships.some((relationship) => relationship.id === selectedRelationship)) setSelectedRelationshipState(null)
  }, [focusedRelationships, layout.nodes, selectedNodeId, selectedRelationship])
  ...
  return <div className={`graph-investigation graph-${model.complexity}`}>
    <div className="graph-context">
      ...
      <div><span className="label">Evidence analyzed</span><strong>{model.transactionCount} transactions</strong></div>
    ...
    <div className="graph-legend"><span><i className="legend-dot source" />Source</span><span><i className="legend-dot intermediate" />Observed wallet</span>{model.nodes.some((node) => node.type === 'vasp') ? <span><i className="legend-dot vasp" />VASP/entity</span> : null}{model.nodes.some((node) => node.type === 'mixer') ? <span><i className="legend-dot mixer" />Mixer-labelled endpoint</span> : null}{model.nodes.some((node) => node.type === 'cluster_member') ? <span><i className="legend-dot cluster" />Cluster member</span> : null}{model.nodes.some((node) => node.type === 'candidate') ? <span><i className="legend-dot lead" />Investigative lead</span> : null}<span>Click a relationship for its underlying evidence.</span></div>
    {layout.maxLayer > 0 ? <div className="hop-filter" aria-label="Focus graph by hop"><span className="label">Focus hop</span><button type="button" className={activeHop === null ? 'active' : ''} onClick={() => setActiveHop(null)}>All</button>{Array.from({ length: layout.maxLayer }, (_, index) => index + 1).map((hop) => <button key={hop} type="button" className={activeHop === hop ? 'active' : ''} onClick={() => setActiveHop(hop)}>Hop {hop}</button>)}</div> : null}
    {!allVisible ? <div className="graph-notice">Current view: {visibleScope}. Overview includes {visibleRelationships.length} of {model.relationships.length} relationships; the evidence ledger retains all {model.transactionCount} analyzed transactions. <button type="button" onClick={() => setShowAll(true)}>Show all relationships</button></div> : <div className="graph-notice">Current view: {visibleScope}. {model.relationships.length > MAX_OVERVIEW_RELATIONSHIPS ? <button type="button" onClick={() => setShowAll(false)}>Return to focused overview</button> : null}</div>}
    <div className="graph-layout">
      <div className="graph-canvas-wrap">
        <div className="graph-toolbar"><button type="button" aria-label="Zoom out" onClick={() => setZoom((value) => Math.max(.65, value - .15))}>−</button><button type="button" onClick={() => setViewport(fitViewport(layout))}>Fit view</button><button type="button" aria-label="Zoom in" onClick={() => setZoom((value) => Math.min(2.2, value + .15))}>+</button></div>
        <svg ref={svg} className="graph-shell adaptive-graph" viewBox={`0 0 ${layout.width} ${layout.height}`} role="img" aria-label="Directional blockchain wallet relationship graph" onWheel={(event) => { event.preventDefault(); setViewport((value) => zoomViewport(value, layout, event.deltaY < 0 ? .12 : -.12)) }} onPointerDown={(event) => { drag.current = { x: event.clientX, y: event.clientY }; event.currentTarget.setPointerCapture(event.pointerId) }} onPointerMove={(event) => { if (!drag.current || !svg.current) return; const bounds = svg.current.getBoundingClientRect(); const dx = (event.clientX - drag.current.x) * layout.width / bounds.width; const dy = (event.clientY - drag.current.y) * layout.height / bounds.height; setViewport((value) => ({ ...value, x: value.x + dx, y: value.y + dy })); drag.current = { x: event.clientX, y: event.clientY } }} onPointerUp={() => { drag.current = null }} onPointerCancel={() => { drag.current = null }}>
          ...
          <g transform={`translate(${viewport.x} ${viewport.y}) scale(${viewport.zoom})`}>
            ...
            {focusedRelationships.map((relationship) => {
              ...
              const selected = selectedRelationship === relationship.id || relationship.transactions.some((transaction) => String(transaction.tx_hash || '').toLowerCase() === String(selectedTransactionHash || '').toLowerCase()); const connected = !selectedNodeId || relationship.source === selectedNodeId || relationship.target === selectedNodeId; const dx = target.x - source.x; const dy = target.y - source.y
              const parallel = focusedRelationships.filter((item) => item.source === relationship.source && item.target === relationship.target); const parallelIndex = parallel.findIndex((item) => item.id === relationship.id)
              const curve = Math.max(-42, Math.min(42, dy * .16 + (parallelIndex - (parallel.length - 1) / 2) * 14))
              const labelVisible = model.complexity === 'minimal' || (model.complexity === 'medium' && focusedRelationships.length <= 6) || selected
            ...
            {layout.nodes.map((node) => { const point = layout.positions.get(node.id)!; const isSelected = selectedNode === node.id; const showLabel = model.complexity !== 'advanced' || isSelected || node.type !== 'intermediate'; const color = COLORS[node.type] || COLORS.intermediate; return <g key={node.id} className={isSelected ? 'graph-node selected-node' : 'graph-node'} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); setSelectedNode(node.id); setSelectedRelationship(null) }}><circle cx={point.x} cy={point.y} r="28" className="wallet-hit-area" /><circle cx={point.x} cy={point.y} r={isSelected ? 20 : 16} fill={color} className="wallet-node" />{showLabel ? <><text x={point.x} y={point.y + 4} className="wallet-address">{short(node.id)}</text><text x={point.x} y={point.y + 32} className="wallet-meta">{node.type === 'victim' ? 'Source' : `Hop ${node.hop_depth ?? '?'}`}</text></> : null}<title>{`${node.label}: ${node.id}`}</title></g> })}
          </g>
        </svg>
      </div>
